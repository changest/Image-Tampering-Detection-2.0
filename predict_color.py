...
def predict(image_path, output_dir):
    ...
    model_path = "models/color_best.pth"
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=DEVICE))
        print(f"[Color] 加载模型: {model_path}")
    else:
        print(f"[Color] 错误: 未找到模型文件 {model_path}")
        return
    ...
    img_output_dir = os.path.join(output_dir, f"{base_name}_Color")
    ...
    axes[1].set_title(f"Color Result\nMean: {pred_resized.mean():.3f}")
    axes[1].axis('off')
...
"""
    ...
    print(f"[Color] 检测完成! 平均置信度: {pred_resized.mean():.4f}")
    print(f"[Color] 结果保存至: {img_output_dir}")

def main():
    parser = argparse.ArgumentParser(description="Color分支篡改检测")
    parser.add_argument("image", type=str, help="输入图像路径")
    parser.add_argument("--output_dir", type=str, default="outputs", help="输出目录")
    args = parser.parse_args()

    print(f"[Color] 使用设备: {DEVICE}")
